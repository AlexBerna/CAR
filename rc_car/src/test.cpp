...
void fix_callback(const std_msgs::String::ConstPtr& msg, ros::NodeHandle * n, ros::Publisher * pub)
{
  ...
  ROS_INFO("Test received a message from extended_fix");
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber tFix = n.subscribe<std_msgs::String>("tTest", 1000, boost::bind(&fix_callback, _1, &n, &tRSR_pub));
  
  //ros::Publisher tError_pub = n.advertise<Error>("tError", 1000);

  if (n.getParam("Test/watchdog_gps_timer", gps_timer_duration))
  {
    ROS_INFO("Test got param /watchdog_gps_timer: %f\n", gps_timer_duration);
  }
  else
  {
    ROS_ERROR("Test failed to get param /watchdog_gps_timer\n");
  }
